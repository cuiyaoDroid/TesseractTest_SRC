...
import com.xianzhi.tesseract.R;
...
public final class CameraManager {
	...
	Camera.PictureCallback m_PictureCallbackJPG = new Camera.PictureCallback() {
		public void onPictureTaken(byte[] data, Camera c) {
			if (m_ParentMessageHandler == null) {
				return;
			}
	...
	public Rect GetFramingRect(boolean linemode) {
		int border = 10;
		int height = DensityUtil.dip2px(m_Context, 25);
		if (linemode)
			m_FramingRect = new Rect(m_ptScreenResolution.x / 6,
					m_ptScreenResolution.y / 2 - height,
					m_ptScreenResolution.x * 5 / 6, m_ptScreenResolution.y / 2
							+ height);
		else
